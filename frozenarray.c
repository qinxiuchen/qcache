...
/* $Id: frozenarray.c 320687 2011-12-08 18:37:34Z gopalv $ */
...
PHP_QCACHE_API zend_class_entry* frozen_array_get_ce()
{
...
PHP_QCACHE_API zend_object_value frozen_array_new(zend_class_entry *ce TSRMLS_DC)
{
...
zval* frozen_array_copy_zval_child_ptr(zval* dst, zval* src, char* child_name, int persistent, size_t* allocated TSRMLS_DC)
{
	if(!dst)
	{
		if(persistent)
